          ...
          <div class="navbar-nav flex-row order-md-last">
            <div class="nav-item dropdown">
              <a href="#" class="nav-link d-flex lh-1 p-0 px-2" data-bs-toggle="dropdown" aria-label="Open user menu">
                <span class="avatar avatar-sm avatar-initial me-2">{{ strtoupper(mb_substr(Auth::user()->name ?? 'A', 0, 1)) }}</span>
                <div class="d-none d-xl-block ps-2">
                  <div>{{ Auth::user()->name ?? 'Admin' }}</div>
                  <div class="mt-1 small text-secondary">{{ (Auth::user()->is_admin ?? false) ? 'Administrator' : (Auth::user()->role ?? 'User') }}</div>
                </div>
              </a>
